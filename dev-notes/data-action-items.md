...
## Cron job - Daily checks if new edw files are present or class schedule is released.

- what is the timing of edw data refreshes and should only encrypted data exist on server
- [x] figure out what the request response would be for the API and test it with Spring 2021
- how to reconcile having to increment the semester?  you could either run an infinite loop and cycle through the seasons list and not use cron OR use cron and write the current season to a log.  
    - but that introduces another problem, if we increment the season then we have to manually run it to get new api data as more classes get added.  actually these can just be independent processes
- we want to run the pipeline with new edw data but with what target semester and current semester? we always keep the current semester in the RNN training, target semester only effects the API. so we need two counters at all times for the cron job - the current semester and the target semester.  scenarios:
    - (1) if new edw data: we increment the current semester, keep the target semester, train the RNN with the new current semester, and get API data for the target semester
    - (2) if new API data: we keep the current semester, increment the target semester, train the RNN with the current semsester, and get API data for the new target semester
- will this work with the RNN? i.e. will the new eval_semester in config.json definitely exist in course_detail_dict?
    - Yes, course_detail_dict is generated by preprocess.py, which is called before the training which uses config.json
- how to persist target and current semester data?  either persistent env variables or just some text file.  test if you can make persistent env variables work and if you update them from a script that they stay changed
