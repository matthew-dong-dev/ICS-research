...
- we want to run the pipeline with new edw data but with what target semester and current semester? we always keep the current semester in the RNN training, target semester only effects the API. so we need two counters at all times for the cron job - the current semester and the target semester.  scenarios:
    - (1) if new edw data: we increment the current semester, keep the target semester, validate the RNN with the new current semester, and get API data for the same target semester.  Also we anonymize the new data, then encrypt and remove the originals.
    - (2) if new API data: we keep the current semester, increment the target semester, validate the RNN with the same current semsester, and get API data for the new target semester.  Also we must first decrypt old data.
