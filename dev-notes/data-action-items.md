...
- how is curr_semester currently being used in Models?  it is used to keep current semester enrollments when generating course dictionaries, but the current semester should not be used in training
    - `if semester < eval_semester: sequence_list.append(tuple(courses))`
